...
export class ClassesController {
  ...
  @Get()
  @ApiOperation({ summary: 'Danh sách lớp học', description: 'Lấy tất cả các lớp học trong hệ thống.' })
  @ApiResponse({ status: 200, description: 'Lấy danh sách thành công' })
  findAll() { return this.classesService.findAll(); }

  @Get(':id')
  @ApiOperation({ summary: 'Chi tiết lớp học', description: 'Lấy thông tin chi tiết của một lớp học theo ID.' })
  ...
  @Post()
  @ApiOperation({ summary: 'Thêm lớp học mới', description: 'Tạo một lớp học mới thuộc về một khoa.' })
  ...
  @Patch(':id')
  @ApiOperation({ summary: 'Cập nhật lớp học', description: 'Chỉnh sửa thông tin lớp học hiện có.' })
  @ApiParam({ name: 'id', description: 'ID của lớp học', type: Number })
  ...
  @Delete(':id')
  @ApiOperation({ summary: 'Xóa lớp học', description: 'Xóa lớp học khỏi hệ thống.' })
  @ApiParam({ name: 'id', description: 'ID của lớp học', type: Number })
